...
let gen_poly is_pure poly_vars =
  let types = List.map (fun v -> tv v.id_str Coq_type true) poly_vars in
  let f v =
    let nm = v.var_name in
    let inhab = ("Ih_" ^ nm, coq_app inhab (coq_var v.var_name)) in
    let enc = ("Enc_" ^ nm, coq_app enc (coq_var v.var_name)) in
...
let rec coq_term tvar_tbl t =
  ...
  | Tlet (vs, t1, t2) ->
      let vs = List.map (fun x -> coq_var x.id_str) vs in
...
let cfml_term tbl t =
  let rec cfml_term = function
    ...
    | Lift (sym, l) ->
        ...
        let coq_id ts = Coq_var ts.ts_id.id_str in
        hdata (coq_id loc) (coq_app (coq_var sym.ps_name.id_str) (coq_id rep))
    | Wand (l, r) ->
...
let gen_spec triple =
  ...
  let args = List.map gen_args triple.triple_args in
  let all_vars = List.map gen_args triple.triple_vars in
  let dynargs =
    List.map (fun v -> coq_dyn_of v.var_type (coq_var v.var_name)) args
  in
  let trm =
    trm_apps_lifted (coq_var ("_" ^ triple.triple_name.id_str)) dynargs
  in
  ...
  let mk_post (vl, tl) =
    ...
    | [] -> term
    | rets ->
        let nms = List.map (fun x -> Coq_var x.ts_id.id_str) rets in
...
let rec sep_def d =
  ...
  | Type tdef -> (
      ...
      | Alias t ->
          ...
          let poly =
            List.map (fun x -> tv x.id_str Coq_type false) tdef.type_args
          in
          [
  ...
  | Function f -> (
      if !is_stdlib && is_infix f.fun_name then []
      else
...
let sep_defs ~stdlib file =
  ...
  let imports =
    ...
    @ (if !is_stdlib then []
       else [ Coqtop_require_import [ "gospelstdlib_verified" ] ])
    @ [
        Coqtop_require
          [
            "Coq.ZArith.BinInt";
